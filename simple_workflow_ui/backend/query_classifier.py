...
class SemanticQueryClassifier:
    ...
    def __init__(self, embeddings_url: str = "http://192.168.1.81:9002"):
        ...
        logging.info(f"Initialized SemanticQueryClassifier with {len(self.category_examples)} categories")
    ...
    def _compute_category_embeddings(self) -> Dict[str, np.ndarray]:
        ...
        for category, examples in self.category_examples.items():
            try:
                ...
                logging.info(f"Computed embeddings for '{category}' using {len(examples)} examples")
            except Exception as e:
                logging.error(f"Failed to compute embeddings for category '{category}': {e}")
    ...
    def classify_query(self, query: str, threshold: float = 0.3) -> Tuple[str, float]:
        ...
        try:
            ...
            logging.info(f"Query classified as '{best_category}' with confidence {best_score:.3f}")
