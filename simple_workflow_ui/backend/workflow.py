...
class ResearchWorkflow:
    ...
    def _call_llm_and_update_state(
        self, state: WorkflowState, prompt: str
    ) -> WorkflowState:
    ...
    def _extract_relevant_sites(
        ...
        if "results" in search_results:
            ...
            for result in search_results["results"][:5]:
                ...
                if any(
                    domain in url.lower() for domain in high_priority_domains
                ) or any(
                    term in title + snippet
                    for term in ["current", "live", "today", "now", "latest"]
                ):
                    relevant_sites.append(url)
                elif (
                    url and len(relevant_sites) < 3
                ):  # Include other results as fallback
                    relevant_sites.append(url)
        ...
        for site_url in sites:
            try:
                ...
                if scrape_response.status_code == 200:
                    ...
                    print(
                        f"❌ Failed to scrape {site_url}: {scrape_response.status_code}"
                    )
    ...
    def _extract_smart_data(
        self, relevant_sites: List[str], query: str, strategy: str
    ) -> Dict[str, Any]:
            ...
            for site_url in relevant_sites:
                try:
                    ...
                    if scrape_response.status_code == 200:
                        ...
                        text_content = scrape_data.get("text", "")
                        print(
                            f"📄 Scraped {len(text_content)} characters from {site_url}"
                        )

                        # Use LLM strategy to extract relevant data
                        relevant_data = self._extract_data_using_strategy(
                            text_content, site_url, query, strategy
                        )
                        if relevant_data:
                            print(
                                f"✅ Relevant data extracted: {relevant_data[:200]}..."
                            )
    ...
    def _extract_data_using_strategy(
        self, text_content: str, source_url: str, query: str, strategy: str
    ) -> str:
...
CONTEXT: [brief context around each data point]"""
    ...
    def _contains_weather_data(self, text: str, query: str) -> bool:
        ...
        if "mountain view" in query_lower:
            location_mentioned = (
                "mountain view" in text_lower or "mountain_view" in text_lower
            )

        return (
            (has_temp or has_weather) and location_mentioned and len(text.strip()) > 100
        )
    ...
    def _extract_temperature_data(self, text: str, source_url: str) -> str:
        ...
        for i, line in enumerate(text_lines):
            ...
            if (
                len(line_clean) < 3
                or "advertisement" in line_clean.lower()
                or "cookie" in line_clean.lower()
                or len(line_clean) > 200
            ):
                continue
                ...
                if matches:
                    ...
                    for ctx_line in context_lines:
                        ...
                        if (
                            ...
                            and "advertisement" not in clean_line.lower()
                        ):
    ...
    def _format_with_rich(
        self, query: str, analysis: str, plan: str, sources_count: int = 0
    ) -> str:
        ...
        # Title
        title_panel = Panel(
            f"🔍 [bold blue]{query}[/bold blue]", style="blue", padding=(0, 1)
        )
    ...
    def _create_workflow(self) -> StateGraph:
        ...
        def analyze_query_type(state: WorkflowState) -> str:
            ...
            try:
                ...
                print(
                    f"🔍 Query Classification: {category} (confidence: {confidence:.3f})"
                )
...
Keep it short - maximum 3 sentences."""
        ...
        def search_node(state: WorkflowState) -> WorkflowState:
            ...
            try:
                ...
                state["search_results"] = {"error": str(e), "status": "failed"}
                state["error"] = str(e)
        ...
        def realtime_analysis_node(state: WorkflowState) -> WorkflowState:
            ...
            try:
                ...
                relevant_sites = self._extract_relevant_sites(
                    state["search_results"], state["query"]
                )
                print(
                    f"🌐 Found {len(relevant_sites)} relevant sites for: {state['query']}"
                )
...
**Brief Summary:** [One sentence summary]"""
        ...
        def finalize_node(state: WorkflowState) -> WorkflowState:
            ...
            try:
                ...
                state["final_result_html"] = self._format_with_rich(
                    query, analysis, plan, sources_count
                )
...
*Powered by LangGraph distributed AI cluster*"""
    ...
    async def execute(self, query: str) -> Dict[str, Any]:
        ...
        try:
            ...
            return {"success": True, "result": result, "formatted_html": formatted_html}

        except Exception as e:
            return {"success": False, "error": str(e), "result": initial_state}
